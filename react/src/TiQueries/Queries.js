...
export const WSE_FORMAT_QUERY = (run) => `wse(${run})`; // don't place a `;`
...
export const RESTORE_QUERY = (takeAccess, restoreTasks) => `restore(fileName${takeAccess || restoreTasks ? ', {take_access: takeAccess, restore_tasks: restoreTasks}' : ''});`;
...
export const NEW_TOKEN_QUERY = (expirationTime, description) => `new_token(name${expirationTime ? ', expirationTime': ''}${description ? ', description': ''});`;
