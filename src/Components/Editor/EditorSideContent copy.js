...
const EditorSideContent = ({scope, onSetQueryInput, tag}) => {
    ...
    const handleChangeEnumType = (n, c) => () => {
        setView({...view, [`${c}s`]: {open: true, name: n}});
    };
    ...
    const enumNames = [...enums.map(c=>c.name)];
    const customType = view.types.name&&customTypes?customTypes.find(i=>i.name==view.types.name):{};
    const rowsTypes = customType.fields? customType.fields.map(c=>({propertyName: c[0], propertyType: c[1], propertyObject: <AddLink name={c[1]} enumItems={enumNames} typeItems={customTypeNames} onChange={handleChangeEnumType} />, propertyVal: ''})):[];
