...
const Login = ({applicationStore, applicationActions}) => {
    const {loaded, connected, connErr} = applicationStore;
    const [state, setState] = useState(initialState);
    const {showPassword, errors, form} = state;

    const connect = useCallback(applicationActions.connect(form));
...
export default () => (
    <AppContext.Consumer>
        {({store, actions}) => <Login applicationStore={store} applicationActions={actions} />}
    </AppContext.Consumer>
);